...
export const CreatePage: (initPage: keyof typeof sectionData) =>NextPage = (initPage: keyof typeof sectionData) => 
  function NextPageCreation(props) {
    ...
    const handleScrollMenuState = debounce(() => {
      if (menuRef.current && ((menuRef.current as any).scrollWidth - (menuRef.current as any).scrollLeft) + (menuRef.current as any).scrollLeft == (menuRef.current as any).scrollWidth)
    ...
    const handleScroll = debounce(useCallback(() => {
     if (!bodyRef.current) return ;
      ...
      console.log(containerScroll, containerScroll < scrollState?"down": "up", container.scrollHeight)
      if (containerScroll > scrollState) {
        if(Object.keys(sectionData).indexOf(pageSection)!=0)
        handleSectionChange({target: {dataset: {
          section: Object.keys(sectionData)[Object.keys(sectionData).indexOf(pageSection)+1]
        }}})
      } 
      if (containerScroll <= scrollState) {
        
        if(Object.keys(sectionData).indexOf(pageSection)!=Object.keys(sectionData).length-1)
        handleSectionChange({target: {dataset: {
          section: Object.keys(sectionData)[Object.keys(sectionData).indexOf(pageSection)+1]
        }}})
    ...
    }, [pageSection, scrollState]), 1000)

    const ActiveSection = sectionData[pageSection]
    const handleSectionChange = useCallback(({ target }: any) => {
      if(!target.dataset.section) return;
      const currIdx = Object.keys(sectionData).indexOf(pageSection)
      const nextIdx = Object.keys(sectionData).indexOf(target.dataset.section)
      const oldGoDown = currIdx > nextIdx
      if (currIdx === nextIdx)
        return
      setFadeStatus(!oldGoDown ? "fadeOutUp" : "fadeOutDown")
      setTimeout(() => {
        history.pushState({page: target.dataset.section}, target.innerText, target.dataset.section);
